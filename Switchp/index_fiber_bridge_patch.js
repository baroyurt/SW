...
(function () {
  ...
  async function disconnectFiber(panelId, portNumber) {
    showLoading();
    try {
      const resp = await postJson('actions/disconnectFiberPort.php', {
        type: 'fiber_port',
  ...
  async function openFiberEditModal(panelId, portNumber) {
    ...
    const html = `
      ...
      <div style="padding:10px;">
        ...
        <div style="display:flex; gap:10px; margin-top:18px; justify-content:flex-end;">
          <button class="btn btn-secondary" id="fb-cancel-btn" style="background:linear-gradient(135deg,#64748b,#475569);color:#fff;padding:10px 20px;border-radius:8px;">Kapat</button>
    ...
    overlay.querySelector('#fb-cancel-btn').addEventListener('click', () => overlay.remove());
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      showLoading();
      try {
        const res = await postJson('actions/saveFiberPortConnection.php', payload);
