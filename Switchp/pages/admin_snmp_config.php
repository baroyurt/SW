...
function sendTestEmailViaSmtp(string $host, int $port, string $user, string $pass,
    ...
    $htmlBody = chamada_email_template('Test Maili', $testContent);
    $subject  = 'CHAMADA Test maili başarıyla gönderildi!';

    $result = EmailService::sendWith($host, $port, $user, $pass, $from ?: $user, [$to], $subject, $htmlBody);
    if ($result['success']) {
        $result['message'] = "Test maili '{$to}' adresine başarıyla gönderildi!";
    }
    return $result;
}
