...
function getRecentAlarms($conn) {
    ...
    echo json_encode([
        ...
        error_log('getRecentAlarms error: ' . $e->getMessage());
...
function getAlarmCount($conn) {
    ...
    $counts = $result ? $result->fetch_assoc() : ['total'=>0,'critical'=>0,'high'=>0,'medium'=>0,'low'=>0];
    ...
    echo json_encode([
        ...
        error_log('getAlarmCount error: ' . $e->getMessage());
        echo json_encode(['success' => false, 'error' => $e->getMessage(), 'timestamp' => date('Y-m-d H:i:s')]);
    }
}
...
function getPortUpdates($conn) {
    ...
    echo json_encode([
        ...
        error_log('getPortUpdates error: ' . $e->getMessage());
        echo json_encode(['success' => false, 'error' => $e->getMessage(), 'timestamp' => date('Y-m-d H:i:s')]);
    }
}
...
function getDeviceStatus($conn) {
    ...
    $query = "SELECT 
              ...
              status,
              last_poll_time,
              DATEDIFF(SECOND, last_poll_time, GETDATE()) as seconds_since_poll
              FROM snmp_devices
              ORDER BY name";
    ...
    echo json_encode([
        ...
        error_log('getDeviceStatus error: ' . $e->getMessage());
        echo json_encode(['success' => false, 'error' => $e->getMessage(), 'timestamp' => date('Y-m-d H:i:s')]);
    }
}
