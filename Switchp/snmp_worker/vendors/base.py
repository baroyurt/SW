...
class PortInfo:
    ...
    # poe_detecting: True = delivering power (pethPsePortDetectStatus == 3)
    # poe_power_mw:  consumption in milliwatts (RFC 3621 returns milliwatts directly, no conversion needed)
    #                -1 = fault sentinel (CBS350: pethPsePortDetectStatus 4=fault / 6=otherFault)
    # poe_class:     pethPsePortPowerClassifications integer value (raw SNMP enum)
    poe_detecting: Optional[bool] = None   # None = PoE not supported on this port
    poe_power_mw: Optional[int] = None     # milliwatts
    poe_class: Optional[int] = None        # raw enum (Cisco: 0-4 direct; RFC: 1-5 offset)
