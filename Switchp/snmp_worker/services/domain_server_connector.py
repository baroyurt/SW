...
class DomainServerConnector:
    ...
    def _get_pyodbc_connection(self):
        """
        Build a pyodbc connection using Windows Authentication when no
        user/password is configured, or SQL Server Authentication otherwise.
        """
        host = self.db_config.get('host', 'MANAGEENGINE\\SQLEXPRESS')
    ...
    def _lookup_from_registry(self, mac_address: str) -> Optional[Dict]:
        ...
        try:
            ...
            if row:
                ...
                conn.close()
                return {
                    'mac_address': row_dict['mac_address'],
                    'ip_address': row_dict['ip_address'],
                    'device_name': row_dict['device_name'],
                    'user_name': row_dict['user_name'],
                    'location': row_dict.get('location'),
                    'department': row_dict.get('department'),
                    'source': row_dict.get('source', 'registry'),
                    'last_updated': row_dict['updated_at']
                }
            cursor.close()
            conn.close()
