...
class SNMPDevice(Base):
    ...
    system_uptime = Column(BigInteger)  # in seconds
...
class PortStatusData(Base):
    ...
    port_speed = Column(BigInteger)  # in bps
    ...
    # Per-port PoE (pethPsePortTable, RFC 3621) — populated by Python worker
    # poe_detecting: 1 = delivering power (pethPsePortDetectStatus == 3)
    # poe_power_mw:  consumption in milliwatts
    # poe_class:     pethPsePortPowerClassifications raw enum value
    poe_detecting = Column(Boolean)   # True/False; NULL = port not PoE capable
    poe_power_mw  = Column(Integer)   # milliwatts
    poe_class     = Column(Integer)   # raw SNMP enum
