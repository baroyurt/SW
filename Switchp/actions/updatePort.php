...
try {
    ...
    if (isset($data['action']) && $data['action'] === 'reset_all_switches') {
        ...
        ")) {
            ...
            // Also clear stale multi-MAC data from all SNMP worker tables.
            try {
                $conn->query("UPDATE port_snapshot SET mac_addresses = NULL");
                $conn->query("UPDATE port_status_data SET mac_addresses = NULL");
            } catch (Exception $e) {
                error_log("Could not clear port_snapshot/port_status_data mac_addresses on reset_all_switches: " . $e->getMessage());
            }
    ...
    if (isset($data['action']) && $data['action'] === 'reset_all') {
        ...
        if ($resetStmt->execute()) {
            ...
            // Also clear stale multi-MAC data from SNMP worker tables so the next
            // poll cycle does not re-classify ports as HUB/DEVICE based on old FDB data.
            try {
                $clearSnapStmt = $conn->prepare("
                    UPDATE port_snapshot
                    SET mac_addresses = NULL
                    WHERE device_id IN (
                        SELECT sd.id
                        FROM snmp_devices sd
                        INNER JOIN switches s ON sd.name = s.name
                        WHERE s.id = ?
                    )
                ");
                if ($clearSnapStmt) {
                    $clearSnapStmt->bind_param("i", $switchId);
                    $clearSnapStmt->execute();
                    $clearSnapStmt->close();
                }

                $clearStatusStmt = $conn->prepare("
                ...
                ");
                if ($clearStatusStmt) {
                    $clearStatusStmt->bind_param("i", $switchId);
                    $clearStatusStmt->execute();
                    $clearStatusStmt->close();
                }
            } catch (Exception $e) {
                error_log("Could not clear port_snapshot/port_status_data mac_addresses on reset_all: " . $e->getMessage());
            }
    ...
    if ($checkResult->num_rows > 0) {
        ...
        if ($updateStmt->execute()) {
            ...
            // Clear stale multi-MAC data from SNMP worker tables to prevent false
            // hub-port detection on the next poll cycle.
            // When a user edits ports.mac via UI, port_snapshot and port_status_data
            // may still hold a mac_addresses JSON from a previous multi-MAC FDB poll.
            // That stale data would cause is_hub_port = True (len(previous_macs) > 1)
            // on the very next poll, classifying a single-device port as HUB.
            // Clearing mac_addresses here ensures the next snapshot comparison uses
            // only the single primary MAC, preventing the false positive.
            try {
                $clearMacStmt = $conn->prepare("
                    UPDATE port_snapshot
                    SET mac_addresses = NULL
                    WHERE device_id = (
                        SELECT TOP 1 sd.id
                ...
                ");
                if ($clearMacStmt) {
                    $clearMacStmt->bind_param("ii", $switchId, $portNo);
                    $clearMacStmt->execute();
                    $clearMacStmt->close();
                }
                ...
                ");
                if ($clearStatusStmt) {
                    $clearStatusStmt->bind_param("ii", $switchId, $portNo);
                    $clearStatusStmt->execute();
                    $clearStatusStmt->close();
                }
            } catch (Exception $e) {
                error_log("Could not clear port_snapshot/port_status_data mac_addresses: " . $e->getMessage());
            }
