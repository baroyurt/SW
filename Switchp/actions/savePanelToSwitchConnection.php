...
try {
    ...
    if ($panelType === 'patch') {
        $upd = $conn->prepare("UPDATE patch_ports SET status='active', connected_switch_id=?, connected_switch_port=?, connection_details=? WHERE panel_id=? AND port_number=?");
        $upd->bind_param("iisii", $switchId, $switchPort, $connectionDetails, $panelId, $panelPort);
        $upd->execute();
        if ($conn->affected_rows == 0) {
            $ins = $conn->prepare("INSERT INTO patch_ports (panel_id,port_number,status,connected_switch_id,connected_switch_port,connection_details) VALUES (?,?,'active',?,?,?)");
            $ins->bind_param("iiiis", $panelId, $panelPort, $switchId, $switchPort, $connectionDetails);
            $ins->execute(); $ins->close();
        }
        $upd->close();
    } else {
        $upd = $conn->prepare("UPDATE fiber_ports SET status='active', connected_switch_id=?, connected_switch_port=?, connection_details=? WHERE panel_id=? AND port_number=?");
        $upd->bind_param("iisii", $switchId, $switchPort, $connectionDetails, $panelId, $panelPort);
        $upd->execute();
        if ($conn->affected_rows == 0) {
            $ins = $conn->prepare("INSERT INTO fiber_ports (panel_id,port_number,status,connected_switch_id,connected_switch_port,connection_details) VALUES (?,?,'active',?,?,?)");
            $ins->bind_param("iiiis", $panelId, $panelPort, $switchId, $switchPort, $connectionDetails);
            $ins->execute(); $ins->close();
        }
        $upd->close();
