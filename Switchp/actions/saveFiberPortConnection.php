    ...
    if ($sideB['type'] === 'fiber_port') {
            ...
            if (!($prevPanel === $bPanel && $prevPort === $bPort)) {
                ...
                if ($prevRow) {
                    if (intval($prevRow['connected_fiber_panel_id']) === $aPanel && intval($prevRow['connected_fiber_panel_port']) === $aPort) {
                        $prevId = $prevRow['id'];
                        $u = $conn->prepare("UPDATE fiber_ports SET connected_fiber_panel_id = NULL, connected_fiber_panel_port = NULL, connection_type = NULL, status = 'inactive', connection_details = NULL, updated_at = NOW(), sync_version = sync_version + 1 WHERE id = ?");
        ...
        while ($r = $res->fetch_assoc()) {
            if (!(intval($r['panel_id']) === $aPanel && intval($r['port_number']) === $aPort)) {
                $rid = $r['id'];
                $u2 = $conn->prepare("UPDATE fiber_ports SET connected_switch_id = NULL, connected_switch_port = NULL, connection_type = NULL, status = 'inactive', connection_details = NULL, updated_at = NOW(), sync_version = sync_version + 1 WHERE id = ?");
                $u2->bind_param("i", $rid);
                if (!$u2->execute()) { $u2->close(); throw new Exception("Failed clearing other fiber_ports row: " . $conn->error); }
                $u2->close();
                writeHistory($conn, $user, 'switch_to_fiber', 'fiber_panel', $r['panel_id'], $r['port_number'], 'switch', $switchId, $switchPort, 'deleted', $r, null);
            }
        }
        $stmt->close();
        ...
        // history
        writeHistory($conn, $user, 'switch_to_fiber', 'fiber_panel', $aPanel, $aPort, 'switch', $switchId, $switchPort, 'created', $oldA, ['connected_switch_id'=>$switchId,'connected_switch_port'=>$switchPort]);
