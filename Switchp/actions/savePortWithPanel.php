...
try {
    ...
    try {
        ...
        if ($portExists) {
            // Güncelle
            // When clearing a port (type=BOŞ, device=''), also reset hub-related columns
            // so that stale HUB classification and device count don't persist.
            $isReset = ($type === 'BOŞ' && $device === '');
            if ($isReset) {
            ...
            // When a port is fully reset, also clear stale multi-MAC data from SNMP
            // worker tables (port_snapshot, port_status_data) so the next poll does not
            // re-classify the port as HUB based on old mac_addresses JSON.
            if ($isReset) {
                try {
