...
function saveYamlConfig($filename, $config) {
    ...
    foreach ($config['switches'] as $switch) {
        ...
        $yaml .= "    snmp_version: \"3\"\n";
        $yaml .= "    snmp_v3:\n";
        $yaml .= "      username: \"" . ($snmp['username'] ?? 'snmpuser') . "\"\n";
        $yaml .= "      auth_protocol: \"" . ($snmp['auth_protocol'] ?? 'SHA') . "\"\n";
        $yaml .= "      auth_password: \"" . ($snmp['auth_password'] ?? '') . "\"\n";
        $yaml .= "      priv_protocol: \"" . ($snmp['priv_protocol'] ?? 'AES') . "\"\n";
        $yaml .= "      priv_password: \"" . ($snmp['priv_password'] ?? '') . "\"\n\n";
    }
